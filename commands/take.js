                ...
                if(!isNaN(parseInt(input[1])) && parseInt(input[1]) > 0){
                    if(dominion.resources[input[2]] >= parseInt(input[1])){
                        ...
                        if(resourceCount + parseInt(input[1]) <= tools.getPlayerStorage(player)){
                            ...
                            embed.addField("Taking Resources",player.name + " will be taking " + input[1] + " " + input[2] + " from the dominion of " + tools.getDominionName(dominion.id))
                        ...
                        embed.setColor([255,0,0])
                        embed.addField("Not Enough of Resource",tools.getDominionName(dominion.id) + " does not have " + input[1] + " " + input[2] + " to be taken")
                        tools.outputEmbed(message.channel,embed,player) 
                    }
                } else {
                    embed.setColor([255,0,0])
